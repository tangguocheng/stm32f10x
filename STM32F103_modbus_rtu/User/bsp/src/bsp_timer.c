...
/*
	目的用于计算任务的运行时间
	假如每隔500us，这个变量的值＋1，这个值溢出需要24.8551349天。
	GXP,ADD,2016年6月9日16:39:32
		*/
volatile uint32_t Task_run_time_value = 0;
...
/*
*********************************************************************************************************
*	函 数 名: Cumulative_Task_run_time_value()
*	功能说明: 累加这个 变量 Task_run_time_value,用于计算任务运行的时间，最初设定是每隔500us,进一次这个函数
*	形    参:  无
*	返 回 值:  无
*********************************************************************************************************
*/
void Cumulative_Task_run_time_value()
{
	...
	//再次启动定时,定时500us,调用本函数会有误差，请注意
	bsp_StartHardTimer(1,500, Cumulative_Task_run_time_value );
}
...
void bsp_InitHardTimer(void)
{
	...
	/* 配置TIM定时中断 (Update) */
	{
		...
		NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 3;	/* 比串口5优先级高 */
